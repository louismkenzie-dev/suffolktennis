...
import { Loader2, Ticket, Mail, Calendar, MapPin, AlertCircle } from "lucide-react";
...
type EventInfo = { id: string; title: string; event_date: string | null; location: string | null };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (bookingIds.length > 0) {
        const { data: tks } = await db.from("tickets").select("booking_id, qr_token, status").in("booking_id", bookingIds);
      ...
      if (eventIds.length > 0) {
        ...
        const { data: evs } = await db.from("events").select("id, title, event_date, location").in("id", eventIds);
                      ...
                      <CardContent className="py-4 flex flex-wrap items-center justify-between gap-3">
                        ...
                        {qr && b.status === "paid" && (
                          <Button asChild variant="outline">
                            <Link to={`/ticket/${qr}`}><Ticket className="w-4 h-4 mr-1" /> Entry ticket</Link>
                          </Button>
                        )}
                      </CardContent>
                    </Card>
